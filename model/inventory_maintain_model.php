...
public function current_stock(){
    $query = $this->mysqli->query("SELECT category.category_name,model.model_name,brand.brand_name,model.total_quantity,model.reorder_level FROM model INNER JOIN product_list ON model.model_id=product_list.model_id INNER JOIN brand ON product_list.brand_id=brand.brand_id INNER JOIN category ON product_list.category_id=category.category_id GROUP BY model.model_name ORDER BY category.category_name DESC");
    while ($row = $query->fetch_assoc()) {
        $result[] = $row;
    }
  return $result;
