...
public function add_new_product($category_id, $product_cost, $brand_id, $model_id, $quantity,$product_status, $product_date, $serial_number,$item_status, $supplier_id,$warranty){
    ...
    public function view_products(){
        $query = $this->mysqli->query("SELECT category.category_name,items.item_id,items.serial_no,product_list.warrenty,brand.brand_name,model.model_id,model.model_name,model.sales_price FROM items INNER JOIN product_list ON items.p_id=product_list.p_id INNER JOIN category ON product_list.category_id=category.category_id INNER JOIN brand ON product_list.brand_id=brand.brand_id INNER JOIN model ON product_list.model_id=model.model_id WHERE items.item_status=1 GROUP BY category.category_name limit 10");
        while ($row = $query->fetch_assoc()) {
...
public function bill_details_search($row){
    $query = $this->mysqli->query("SELECT  DISTINCT bill.bill_no,customer.cust_name,cust_telephone.telephone_no,bill.date_time from bill INNER JOIN purchase ON bill.bill_no=purchase.bill_no INNER JOIN customer ON purchase.cust_id=customer.cust_id INNER JOIN cust_telephone ON customer.cust_id=cust_telephone.cust_id WHERE bill.bill_no LIKE  '%" . $row . "%' OR customer.cust_name LIKE  '%" . $row . "%' OR cust_telephone.telephone_no LIKE  '%" . $row . "%' OR bill.date_time LIKE  '%" . $row . "%'");
    if ($query->num_rows > 0){ 
        while ($row = $query->fetch_assoc()) {
            $result[] = $row;
        }
 ...
 public function check_brand_equal_model($model){
    $query = $this->mysqli->query("SELECT count(brand.brand_name),brand.brand_name from product_list INNER JOIN brand ON product_list.brand_id=brand.brand_id INNER JOIN model ON product_list.model_id=model.model_id WHERE model.model_name='".$model."' ");
    if ($query->num_rows > 0){ 
    while ($row = $query->fetch_assoc()) {
